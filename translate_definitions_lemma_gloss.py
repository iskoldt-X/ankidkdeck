...
def try_parse_json(raw: str):
    """
    Strip JSON code fences and attempt to parse the content.
...
def translate_definitions_for_entry(headword: str, defs: list[str]) -> dict:
    ...
    Each output should be a JSON object with fields {lemma, gloss}.
    Accepts various JSON schemas. Returns a mapping:
      original definition -> {lemma, gloss}
    """
    system = (
        f"You are a professional dictionary editor translating Danish definitions into {TARGET_LANG}.\n"
        "Do NOT translate the headword; keep it exactly as provided.\n"
        "For each definition, output a JSON object with two fields:\n"
        f"  lemma — the concise {TARGET_LANG} word or fixed phrase serving as headword, preserving original part of speech;\n"
        f"    • if a natural, commonly‑used single word or fixed phrase exists, prefer it over a literal description;\n"
